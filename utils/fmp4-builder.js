...
export class FMP4Builder {
   ...
   * Dùng tfdt của fragment đầu và cuối + duration của fragment cuối, với timescale ĐÚNG của track
   * (thay vì hard-code 90000).
   * @param {Array} pairs - mảng { moofData, mdatData, time, trunDuration }
   * @param {number} timescale - timescale thực tế của track (mặc định 90000 nếu không xác định được)
   * @returns {number} duration tính bằng giây, hoặc 0 nếu không tính được
   */
  static _calcActualDuration(pairs, timescale = 90000) {
    ...
    const totalUnits = (lastTime - firstTime) + lastDur;
    const durationSec = totalUnits / timescale;
    console.log(`[VDP-fMP4] ⏱️ Tính duration từ tfdt: firstTime=${firstTime}, lastTime=${lastTime}, lastDur=${lastDur}, ts=${timescale}, total=${totalUnits} units → ${durationSec.toFixed(3)}s`);
    return durationSec;
  }
  ...
  static async build({ buffers, initSegmentUrl, fetchRawFn, clipRange = null }) {
    ...
    // 1. Tải init segment theo URL nếu có
    if (initSegmentUrl && typeof fetchRawFn === 'function') {
      try {
    ...
    if (clipRange && Number.isFinite(clipRange.end - clipRange.start) && clipRange.end > clipRange.start) {
      ...
      durationSec = this._calcActualDuration(pairs, videoTimescale);
      if (!durationSec || durationSec <= 0) {
